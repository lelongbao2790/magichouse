...
### Initiative Folder
Every distinct engagement (a new request that isn't a continuation of an existing one) gets its own
folder: `aidlc-docs/{initiative-slug}/`, containing that initiative's INCEPTION, CONSTRUCTION, and
OPERATIONS artifacts together. A repo may accumulate multiple initiative folders over time. See
[workspace-detection.md](../inception/workspace-detection.md) Step 0 for how the slug is derived and
how new-vs-resume is decided.

### Plans
Documents with checkboxes and questions that guide execution.
- Located in `aidlc-docs/{initiative-slug}/plans/`
- Examples: `story-generation-plan.md`, `unit-of-work-plan.md`

### Artifacts
Generated outputs from executing plans.
- Located in various `aidlc-docs/{initiative-slug}/` subdirectories
- Examples: `requirements.md`, `stories.md`, `design.md`

### State Files
Files tracking workflow progress and status.
- `aidlc-state.md`: Overall workflow state (machine-oriented, for AI resumption)
- `HANDOVER.md`: Human-oriented briefing kept in sync with `aidlc-state.md` — see
